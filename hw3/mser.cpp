...
MSER_1::Region::Region(int level, int pixel) : level_(level), pixel_(pixel), area_(0),
variation_(numeric_limits<double>::infinity()), stable_(false), parent_(0), child_(0), next_(0)
{
	fill_n(moments_, 5, 0.0);
}

inline void MSER_1::Region::accumulate(int x, int y)
{
	++area_;
	moments_[0] += x;
	moments_[1] += y;
...
void MSER_1::Region::merge(Region * child)
{
...
void MSER_1::Region::process(int delta, int minArea, int maxArea, double maxVariation)
{
...
bool MSER_1::Region::check(double variation, int area) const
{
	if (area_ <= area)
		return true;
...
void MSER_1::Region::save(double minDiversity, vector<Region> & regions)
{
	if (stable_) {
...
void MSER_1::Region::detect(int delta, int minArea, int maxArea, double maxVariation,
...
MSER_1::MSER_1(int delta, double minArea, double maxArea, double maxVariation, double minDiversity,
...
void MSER_1::operator()(const uint8_t * bits, int width, int height, vector<Region> & regions)
{
	...
	regionStack.push_back(new (&pool_[poolIndex_++]) Region);//先存一層在++
...
step_3:
	...
	for (;;) {
		...
		for (; curEdge < (eight_ ? 8 : 4); ++curEdge) {
			...
			if (neighborPixel != curPixel && !accessible[neighborPixel]) {
				...
				if (neighborLevel >= curLevel) {
					boundaryPixels[neighborLevel].push_back(neighborPixel << 4);//x16
					//vector<int>::iterator iter;
					//vector<int>::iterator iter = boundaryPixels[neighborLevel].begin();
					//cout << "iter=" << *iter << endl;
					//for(int ix = 0; iter != boundaryPixels[neighborLevel].end(); ++iter, ++ix){
					//	cout << *iter << endl;
					//}
					//cout << "boundaryPixels["<<neighborLevel<<"]=" << (boundaryPixels[neighborLevel].back()>>4) << endl;
		...
		boundaryPixels[priority].pop_back(); //丟掉
...
void MSER_1::processStack(int newPixelGreyLevel, int pixel, vector<Region *> & regionStack)
{
...
ptrdiff_t MSER_1::doublePool(vector<Region *> & regionStack)
{
